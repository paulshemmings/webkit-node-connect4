var Game = {
	...
	calculateMove : function() {
		...
		for (var x=0; x<grid.rows.length; x++) {			
			...
			var playWeight = this.weighMove(this.currentPlayerIndex, x, y);
			var blockWeight = this.weighMove(opponent, x, y);
			...
			playWeight -= this.weighMove(this.currentPlayerIndex, x, y-1) * 0.8;
			blockWeight -= this.weighMove(opponent, x, y-1) * 0.8;
	...
	playMove : function(x, y) {
		...
		if (gameOver) {
			this.gameStatus = this.GameStatusEnum.OVER;
			this.writeStatus('game over.', false, this.LoggingLevelEnum.INFO);
			this.writeStatus(this.players[this.currentPlayerIndex].getId() + ' wins', false, this.LoggingLevelEnum.INFO);
			return;
		}
	...
	addPlayer : function(isAI) {
		...
		if (this.players.length < this.REQUIRED_PLAYERS) {
			var playerIdentifier = 'Player ' + (this.players.length+1);
